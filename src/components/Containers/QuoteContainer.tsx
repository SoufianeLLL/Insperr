...
const QuoteContainer = ({ id, quote, type='custom', withAuthor=true, classes=null, callback=null, mutate=null }) => {
	...
	if (type === 'custom') {
		return (
			<div className={`quote_${quote?.id} inline-block mb-7 w-full max-w-2xl rounded-2xl shadow hover:shadow-lg border border-slate-100 p-5 md:p-8 bg-white`}>
				<div className="w-full">
						...
						<div className="w-full shrink">
							{quote?.username ? <>
								...
								<div className="text-base -mt-1 text-slate-600 w-full">@{quote?.username}</div>
									...
									<div className="w-full h-6 rounded-full bg-zinc-100"></div>
									<div className="w-10/12 mt-2 h-4 rounded-full bg-zinc-100"></div>
								</div> }
						</div>
					</div>
					...
					<div className="w-full inline-block mt-3 flex items-center gap-x-12">
						...
						<div className="h-0 w-0"></div>
						{quote?.tweet_id && quote?.status === 'publish' && 
							<Tooltip className="order-last" content="Published on Twitter">
								<Link href={`https://twitter.com/${quote?.username}/status/${quote?.tweet_id}`} target="_blank"><svg className="cursor-pointer w-5 h-5 text-slate-200" fill="currentColor" clipRule="evenodd" fillRule="evenodd" strokeLinejoin="round" strokeMiterlimit="2" viewBox="0 0 24 24"><path d="m11.998 2.005c5.517 0 9.997 4.48 9.997 9.997 0 5.518-4.48 9.998-9.997 9.998-5.518 0-9.998-4.48-9.998-9.998 0-5.517 4.48-9.997 9.998-9.997zm-5.049 10.386 3.851 3.43c.142.128.321.19.499.19.202 0 .405-.081.552-.242l5.953-6.509c.131-.143.196-.323.196-.502 0-.41-.331-.747-.748-.747-.204 0-.405.082-.554.243l-5.453 5.962-3.298-2.938c-.144-.127-.321-.19-.499-.19-.415 0-.748.335-.748.746 0 .205.084.409.249.557z" fillRule="nonzero"/></svg></Link>
							</Tooltip>} */}
					</div>
				</div>
			</div>
		...
		<div className={`${classes} quote_${quote?.id} p-quote bg-white shadow hover:shadow-lg border border-slate-100 p-5 md:p-8 w-full rounded-lg relative overflow-hidden mb-6 transition duration-200`}>
			...
			<div style={{ zIndex: 1 }} className="w-full relative">
				...
				<div className="mb-2 w-full text-base">
					<Link href={`/authors/${sluging(quote?.author_name)}`} className="inline-block opacity-80">
						<AuthorContainer author={quote?.author_name} /></Link>
				</div>}
				...
				{topics && 
					<div className="w-full mt-2">
						{topics?.map((topic, i) => {
							return (
								<Link key={i} href={`topics/${topic?.toLowerCase()}`} className="inline-block text-primary-500 float-left mr-3 mb-1 text-sm">
