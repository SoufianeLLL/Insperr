...
const QuoteContainer = ({ id, quote, type='custom', withAuthor=true, classes=null, callback }) => {
	...
	const addQuote_toBookmark = async (quote, id) => {
		...
		if (!isLoading) {
			if (user) {
				try {
					const result = await fetch('/api/saves?action=Insert', {
						method: 'POST',
						headers: {
	...
	return <>
		<div className={`${classes} p-quote bg-white shadow hover:shadow-lg border border-slate-100 p-5 md:p-8 w-full rounded-lg relative overflow-hidden mb-6 transition duration-200`}>
			...
			:
				<div onClick={() => addQuote_toBookmark(quote, id )} style={{ zIndex: 2 }} className="save text-slate-200 cursor-pointer hover:text-primary-500 absolute top-2 right-2 py-2 px-2"><svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"><path d="M19 24l-7-6-7 6v-24h14v24z"/></svg></div>
			}
			<div style={{ zIndex: 1 }} className="w-full relative">
				{withAuthor &&
				<div className="mb-2 w-full text-base">
					{type === 'custom' ? 
						<Link href={`/user/${quote?.users?.username}`}><a className="inline-block opacity-80">
