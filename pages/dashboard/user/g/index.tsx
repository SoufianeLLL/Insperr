...
const GenerateQuotesPage = () => {
	...
	const [target, setTarget] = useState({ category: null, author: null, quota: 1 })
									...
									<div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 gap-3 text-base">
										{profiles?.length && profiles?.map((author) => {
											...
											author={author?.name} classes={{ 
												more: `${target?.author?.toLowerCase() === author?.name?.toLowerCase() ? 'border-blue-500' : 'border-white'}`, 
						...
						<div className="flex-none md:w-1/3 w-full md:mt-0 mt-10">
							<div className="shrink w-full bg-white p-5 rounded-xl">
								<div className="category-col w-full">
									<div className="w-full text-zinc-400 text-base mb-2">Salect topic:</div>
									<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 w-full gap-3 text-base">
										{Topics?.map ((topic) => {
											return <div onClick={() => setTarget({ ...target, category: topic?.name?.toLowerCase() })} className={`${target?.category?.toLowerCase() === topic?.name?.toLowerCase() ? 'text-blue-500 font-semibold' : ''} cursor-pointer transition duration-200 hover:underline`}>{topic?.name}</div>
										})}
									</div>
								</div>
							</div>
							...
							<div onClick={() => (target?.category && target?.author) && startGenerating()} className={`${target?.category && target?.author ? 'bg-primary-500 text-white hover:bg-primary-700' : 'bg-zinc-300 text-white'} w-full cursor-pointer transition duration-200 py-3 px-4 text-center rounded-xl mt-4`}>
